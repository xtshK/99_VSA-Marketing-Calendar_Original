...
async function loadData() {
  try { var r=await window.storage.get(STORAGE_KEY); return r?JSON.parse(r.value):[]; } catch(e){ return []; }
}
async function saveData(items) {
  try { await window.storage.set(STORAGE_KEY,JSON.stringify(items)); } catch(e){}
}
...
async function callClaude(content, isPdf) {
  ...
  var b = d.content&&d.content.find(function(b){return b.type==="text";});
  return b?b.text:"";
}
